...
(function($) {
	...
	$.alerts = {
		...
		horizontalOffset: 100,                // horizontal offset of the dialog from center screen, in pixels/
		...
		_show: function(title, msg, value, type, callback) {
			...
			$("BODY").append(
			  '<div id="popup_container">'  + '<div id="popup_border">' +
			...
			$("#popup_message").text('<div class="poploading" id="poploading" ><div><img border=0 src=/images/loader3.gif></div></div>'+msg);
			...
			$('body').css({'height':'100%','overflow':'hidden','padding-right':'17px'})
			$.alerts._reposition();
			$.alerts._maintainPosition(true);
			
			switch( type ) {
				case 'alert':
					$('#popup_container iframe').css('opacity','0')
					$('#popup_container iframe').load(function(){
						$(this).css('opacity','1')
					})
					$("#popup_message").after('');
		...
		_overlay: function(status) {
			switch( status ) {
				case 'show':
					$.alerts._overlay('hide');
					$("BODY").append('<div id="popup_overlay" onclick="CloseDIV();"></div>');
					$("#popup_overlay").css({
						position: 'absolute',
						zIndex: 99998,
						top: '0px',
...
function CloseDIV(){
try{
Cookies.remove('ProdID');
Cookies.remove('prodid');
Cookies.remove('h');
Cookies.remove('DoType');
Cookies.remove('dotype');
}catch(err){
...
try{ 
	...
	document.getElementById("popup_overlay").removeNode(true);
	document.body.style.overflow = 'auto'
	document.body.style.paddingRight = '0px'
} 
}